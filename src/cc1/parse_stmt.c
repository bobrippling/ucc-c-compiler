...
static asm_param **parse_asm_inout(symtable *scope, int is_output)
{
	asm_param **inouts;
	...
	do{
		struct cstring *cstr;
		char *constraints;
		asm_param *io = umalloc(sizeof *io);
		...
		if(cstr)
			constraints = cstring_detach(cstr);
		else
			constraints = ustrdup("");
...
static char **parse_asm_clobbers(void)
{
	...
	while(curtok == token_string){
		struct cstring *cstr = parse_asciz_str();

		EAT(token_string); /* error emitted here */

		if(cstr){
			char *s = cstring_detach(cstr);
			dynarray_add(&ret, s);
		}
...
static stmt *parse_asm(symtable *scope)
{
	stmt *asm_st;
	asm_args *bits;
	...
	cstr = parse_asciz_str();
	if(cstr)
		bits->cmd = cstring_detach(cstr);
	EAT(token_string); /* error emitted here */

	/* output operands */
	if((bits->extended = accept(token_colon))){
		bits->params = parse_asm_inout(scope, 1);

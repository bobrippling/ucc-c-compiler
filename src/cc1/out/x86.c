		...
		const struct vreg *reg /* may be null */)
{
	...
	if(high_bit > AS_MAX_INTMOV_BIT){
		...
		UCC_ASSERT(!IS_32_BIT(), "TODO: 32-bit 64-literal loads "
				"(%lld, high bit %d)", from->bits.val_i, high_bit);
		...
		const char *sfrom, const char *sto)
{
	...
	/* FIXME: long long on 32-bit */
	out_asm(octx, "cvt%s2%s%s %s, %%%s # tto=%s, int_ty=%s",
			/*truncate ? "t" : "",*/
			sfrom, sto,

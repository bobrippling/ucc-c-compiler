		...
		const struct specvars *vars)
{
	...
	if(!strcmp(varname, "defaultlibs")){
		*varvalue = vars->defaultlibs;
		return 1;
	}
	if(!strcmp(varname, "stdlibinc")){
		*varvalue = vars->stdlibinc;
		return 1;
	}
	if(!strcmp(varname, "builtininc")){
		*varvalue = vars->builtininc;
		return 1;
	}
...
void spec_default(struct specopts *opts)
{
	opts->as = ustrdup("as");
	opts->ld = ustrdup("ld");
}

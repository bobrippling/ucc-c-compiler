...
typedef struct { int a; char *p; } Inner;
struct { int x; const Inner; } outer;

Inner *i = &outer; // should warn about `const Inner` --> `Inner`
//     ^ this should be (char*)&outer+offsetof(typeof(outer), Inner), which it isn't

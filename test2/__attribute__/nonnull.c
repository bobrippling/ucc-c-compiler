...
r(int i, int j) __attribute__((nonnull(0))); // CHECK: /nonnull attribute applied to non-pointer argument 'int'/

main()
{
	f((void *)0); // CHECK: /null passed where non-null required (arg 1)/
}
